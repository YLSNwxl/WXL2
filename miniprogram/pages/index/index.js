...
Page({
  ...
  onLoad: function () {
    ...
    city_lai_long0:app.globalData.city_lai_long,
    
    });
    console.log(this.data.city_lai_long0);
    this.getWeather();
  },
  //获得输入框中的文字
  inputing: function (e) {
  ...
  bindSearch: function () {
    ...
    if (that.data.City == '') {
      ...
      var BMAp = new bMap.BMapWX({
        ak: 'GaH69EO5AW2I4p4Ay62uRpyOSh8TaDlf'
      });
      var fail0 = function (data) {
        console.log(data);
      }
      var success0 = function (data) {
        console.log(data);
        var wxMarkerData = data.wxMarkerData;
        that.data.city_lai_long0 = wxMarkerData[0].longitude + ',' + wxMarkerData[0].latitude;
        console.log(that.data.city_lai_long0);
        that.getWeather();
      }
      // 发起geocoding检索请求 
      BMAp.geocoding({
  ...
  getWeather: function () {
    ...
    // 发起weather请求
    BMap.weather({
      location: that.data.city_lai_long0,
      fail: fail,
      success: success
    }); 
